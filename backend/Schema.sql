CREATE DATABASE IF NOT EXISTS grade_predictor;
USE grade_predictor;
...
-- Login table for students
CREATE TABLE StudentUsers (
    user_id VARCHAR(20) PRIMARY KEY,
    password_hash TEXT NOT NULL,
    email VARCHAR(100) UNIQUE NOT NULL,
    FOREIGN KEY (user_id) REFERENCES Students(RollNumber) ON DELETE CASCADE
);

-- Login table for instructors
CREATE TABLE InstructorUsers (
    user_id INT PRIMARY KEY,
    password_hash TEXT NOT NULL,
    email VARCHAR(100) UNIQUE NOT NULL,
    FOREIGN KEY (user_id) REFERENCES Instructors(InstructorID) ON DELETE CASCADE
);
